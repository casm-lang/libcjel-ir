...
static libpass::PassRegistration< NovelToC11Pass > PASS(
    "NOVEL IR to C11", "generates C11 code out of the NOVEL IR", 0, 0 );
    ...
    if( value.getLinkage().size() > 0 )
    {
        ...
        for( Value* linkage : value.getLinkage() )
        {
            ...
            if( origin )
            {
                ...
                // TODO: FIXME: PPA: HACK: needs better implemented directly in
                // the NOVEL model
...
void NovelToC11Pass::visit_prolog( Reference& value )
{
    ...
    fprintf( stream, "%s%s %s // %s %s%s", getTypeString( value ),
        ( ( value.getType()->getIDKind() == Type::STRUCTURE
              or value.isOutput() )
                ? "*"
                : "" ),
...
void NovelToC11Pass::visit_prolog( Variable& value )
{
    ...
    fprintf( stream, "const %s %s = %lu; // '%s'\n", getTypeString( n ),
        value.getLabel(), value.getAllocationID()->getValue()[ 0 ],
        value.getIdent() );
...
static void non_trivial_statement( Scope& value )
{
    ...
    if( Value::isa< BranchStatement >( parent ) )
    {
        ...
        assert( expr->getType()->getIDKind() == Type::BIT
                && expr->getType()->getBitsize() == 1 );
...
void NovelToC11Pass::visit_prolog( CallInstruction& value )
{
    fprintf( stream, "%s%s( ", indention( value ),
        ( Value::isa< CastInstruction >( value.getValue( 0 ) ) )
    ...
    for( auto v : value.getValues() )
    {
        ...
        if( Value::isa< Instruction >( v )
            and cnt > cu->getInParameters().size() )
        {
            kind = "*)&";
        }
        else if( Value::isa< Instruction >( v )
                 and v->getType()->getIDKind() == Type::STRUCTURE )
        {
            kind = "*)&";
        }
        ...
        // ) )
        // {
        // 	if( v->getType()->getIDKind() == Type::STRUCTURE ) // cnt <=
...
void NovelToC11Pass::visit_prolog( IdCallInstruction& value )
{
    ...
    for( auto v : value.getValues() )
    {
        ...
        const char* kind = ")";
        if( Value::isa< Instruction >( v )
            and cnt > cs->getInParameters().size() )
        {
            kind = "*)&";
        }
        else if( Value::isa< Instruction >( v )
    ...
    for( Value* v : m->get< Function >() )
    {
        ...
        fprintf( stream, "%s    case %lu: { %s( %s ); break; }\n", indent,
            cu->getAllocationID()->getValue()[ 0 ], cu->getName(),
            args.c_str() );
    }
...
void NovelToC11Pass::visit_prolog( ExtractInstruction& value )
{
    ...
    if( Value::isa< AllocInstruction >( base_ )
        and Value::isa< Structure >( offset_ ) )
    {
    ...
    assert( Value::isa< Reference >( base_ )
            or Value::isa< CastInstruction >( base_ ) );
