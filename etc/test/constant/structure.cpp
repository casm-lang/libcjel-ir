...
{
    auto t0 = libstdhl::Memory::make< BitType >( 10 );

    auto s0 = libstdhl::Memory::make< StructureType >( libstdhl::Memory::make< Structure >(
        "s0", std::initializer_list< StructureElement >{ { t0, "field" } } ) );

    auto c0 = libstdhl::Memory::make< StructureConstant >(
